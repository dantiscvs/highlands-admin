-- 012: expose per-day GPX URLs (and descent) on the public share view so the
-- participant page can draw real elevation profiles, not just per-day bars.
-- Rebuilt from 006 with 'gpxUrl' added to the days payload.
